...
def processArea(image):
    ...
    # polygons = np.array([[(0, image.shape[0]), (250, 290), (180, 320), (image.shape[1], image.shape[0])]]) #for 1
...
def detectLanes(img):
    ...
    cropped_image = processArea(sobel_image)
...
path_to_video = glob.glob('TestVideo_2/*.bmp')

for i,image_path in enumerate(path_to_video):
    print(image_path)
    image = mpimg.imread(image_path)
    img = np.copy(image)
    result = detectLanes(img)
    #mpimg.imsave('Output3/'+image_path[12:-4]+'_out.jpg', result)'''
...
def detectLanes(img, threshold = 100):
    ...
    # combine the line image and the color image
